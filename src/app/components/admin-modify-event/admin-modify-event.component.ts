...
@Component({
  selector: 'app-admin-modify-event',
  templateUrl: './admin-modify-event.component.html',
  styleUrls: ['./admin-modify-event.component.scss']
})
export class AdminModifyEventComponent implements OnInit {
  ...
  constructor(private router: Router, private activatedRoute: ActivatedRoute, private http: HttpClient, private eventService: EventService, private statusService : StatusService) {
   ...
   modify(){
    ...
    if(data.location!=""&&data.name!=""&&data.start_date<data.end_date){
      this.eventService.update(data).subscribe((response:any)=>{
